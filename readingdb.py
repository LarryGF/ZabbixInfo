...
def db_connect():    
	...
	global session,hosts,items,trends
	noError = False
	count = int(0)
	while noError==False:
		try:
...
def search_host(hostname: str):
	"""
	Receives a hostname and returns

	(hosts.id, hosts.name)

	where hostname is a substring of hosts.name
	"""
...
def get_items(hostid: int):
	"""
	Receives a hostid and returns

	(items.hostid, items.itemid, items.name)

	where items.hostid == hostid
	"""
